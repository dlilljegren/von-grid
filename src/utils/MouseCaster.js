...
export class MouseCaster{
	...
	update() {
		...
		if (intersects.length > 0) {
			...
			hit = intersects[0];
			if(hit.name=="HooverCell"){
				console.error("HIT the hoover cell bad");
			}
			...
			if (this.pickedObject != obj) {
				...
				//this.signal.dispatch(vg.MouseCaster.OVER, null);
			}
	...
	_onDocumentMouseUp(evt) {
		...
		if (this.selectedObject && this.pickedObject && this.selectedObject.uniqueID === this.pickedObject.uniqueID) {
			...
			//this.signal.dispatch(vg.MouseCaster.CLICK,null);//added by david to capture clicks on empty cells
		}
